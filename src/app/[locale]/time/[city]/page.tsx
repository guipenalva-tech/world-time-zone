...
/** Resolves a narrative sentence's values for interpolation: the
 * `hemisphereKey` placeholder (a translation key name chosen by
 * cityNarrative.ts, e.g. "hemisphereNorthWord") is swapped for the
 * actual translated word under the `hemisphere` param the message
 * template expects. Every other value passes through unchanged. */
function resolveNarrativeValues(
  t: (key: string, values?: Record<string, string | number>) => string,
  values: NarrativeSentence["values"],
): Record<string, string | number> {
  if (typeof values.hemisphereKey === "string") {
    const { hemisphereKey, ...rest } = values;
    return { ...rest, hemisphere: t(hemisphereKey) };
  }
  return values;
}
...
export default async function CityTimePage({
  ...
  const narrativeText = narrativeSentences.map((s) =>
    t(s.key, resolveNarrativeValues(t, s.values)),
  );
  ...
  faqItems.push({
    question: t("faqQ1", { city: city.name }),
    answer: t("faqA1", {
  ...
  faqItems.push({
    question: t("faqQ2", { city: city.name }),
    answer: facts.dstTransition
      ? t("faqA2Yes", { city: city.name })
      : t("faqA2No", {
          city: city.name,
          offset: facts.zoneInfo.offsetFormatted,
        }),
  ...
  faqItems.push({
    question: t("faqQ4", { city: city.name }),
    answer: t("faqA4", {
      city: city.name,
      offset: facts.zoneInfo.offsetFormatted,
      abbreviation: facts.zoneInfo.abbreviation,
  ...
  faqItems.push({
    question: t("faqQ5", { city: city.name }),
    answer:
      facts.sun.daylightState === "alwaysAbove"
        ? t("faqA5Midnight", { city: city.name })
        : facts.sun.daylightState === "alwaysBelow"
          ? t("faqA5Polar", { city: city.name })
          : t("faqA5", {
              city: city.name,
  ...
  faqItems.push({
    question: t("faqQ6", { city: city.name }),
    answer: t("faqA6", {
      city: city.name,
      population: new Intl.NumberFormat(locale).format(city.population),
    }),
  });
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-8 px-4 pb-24 pt-6 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-2xl font-semibold sm:text-3xl">
              {t("heading", { city: city.name })}
            </h1>
            <p className="truncate text-sm text-foreground/60">
              {city.country} · {facts.zoneInfo.offsetFormatted} (
      ...
      >
        ...
        {facts.dstTransition ? (
          <div className="flex flex-col gap-1 text-sm">
            <p>{t("dstObservesIntro", { city: city.name })}</p>
                ...
                return (
                  <tr key={ref.city.id} className="border-t border-border">
                    <td className="py-1.5 pr-2">
                      ...
                      >
                        {getFlagEmoji(ref.city.countryCode)} {ref.city.name}
                      </Link>
                    </td>
      ...
      >
        ...
        <p className="mb-3 text-xs text-foreground/50">
          {t("meetingSubtitle", { city: city.name })}
        </p>
                ...
                <tr key={row.hub.id} className="border-t border-border">
                  <td className="py-1.5 pr-2">
                    <Link
                      ...
                      className="hover:text-primary hover:underline"
                    >
                      {getFlagEmoji(row.hub.countryCode)} {row.hub.name}
                    </Link>
                  </td>
                  <td className="py-1.5 pr-2 tabular-nums">
                    {tFlights("distanceKm", { km: row.distanceKm })}
                  </td>
                  <td className="py-1.5 pr-2 tabular-nums">
                  ...
                  <td className="py-1.5 pr-2 tabular-nums">
                    ${row.priceMinUsd}–${row.priceMaxUsd}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            {facts.sameCountryCities.length > 0 && (
              <div>
                <p className="mb-1.5 text-xs font-semibold text-foreground/60">
                  {t("relatedSameCountry", { country: city.country })}
                </p>
                <ul className="flex flex-col gap-1 text-sm">
